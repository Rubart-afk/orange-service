...
window.addEventListener('hashchange',route);window.addEventListener('DOMContentLoaded',route);if(location.hostname!=='rubart-afk.github.io')api('/api/me').then(data=>{user=data.user;updateAccount();if(['','#home','#profile','#email-accounts','#mail-health','#sequences','#companies'].includes(location.hash))route();}).catch(()=>{});
